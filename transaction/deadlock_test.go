...
func TestDeadlockDetection(t *testing.T) {
	...
	config.DeadlockCheckInterval = 50 * time.Millisecond
	config.LockTimeout = 50 * time.Millisecond // 更短的超时时间

	manager, err := NewLockManager(config)
	if err != nil {
	...
	// 获取一个锁
	err = manager.AcquireLock(txn1, key1, ExclusiveLock)
	if err != nil {
...
func TestDeadlockVictimSelection(t *testing.T) {
	...
	for i, txn := range txns {
		err := manager.AcquireLock(txn, keys[i], ExclusiveLock)
		if err != nil {
...
func TestDeadlockPrevention(t *testing.T) {
	...
	// 事务2获取key2
	err = manager.AcquireLock(txn2, key2, ExclusiveLock)
	if err != nil {
	...
	// 测试死锁预防
	// 这里的预防机制会检测到潜在的死锁
	err = manager.PreventDeadlock(txn1, key2, ExclusiveLock)
	if err != nil {
...
func TestLockTimeoutOptimization(t *testing.T) {
	...
	err = manager.AcquireLock(txn1, key, ExclusiveLock)
	if err != nil {
...
func TestWaitGraphOperations(t *testing.T) {
	...
	// 在后台尝试获取锁（会创建等待关系）
	go func() {
		manager.AcquireLockWithTimeout(txn2, key1, ExclusiveLock, 50*time.Millisecond)
	}()
...
func TestDeadlockAnalysis(t *testing.T) {
	...
	err = manager.AcquireLock(txn, key, ExclusiveLock)
	if err != nil {
		t.Fatalf("Failed to acquire lock: %v", err)
	}
