...
func TestDebugTransactionWAL(t *testing.T) {
	...
	// 获取初始统计
	initialStats := txnMgr.txnWAL.GetStatistics()
	...
	// 检查事务创建后的统计
	afterBeginStats := txnMgr.txnWAL.GetStatistics()
	t.Logf("After BeginTransaction WAL statistics: %+v", afterBeginStats)

	if afterBeginStats.TotalRecords > initialStats.TotalRecords {
		t.Logf("✓ Transaction begin was recorded in WAL")
	} else {
		t.Logf("✗ Transaction begin was NOT recorded in WAL")
	}
	...
	// 检查Put操作后的统计
	afterPutStats := txnMgr.txnWAL.GetStatistics()
	t.Logf("After Put operation WAL statistics: %+v", afterPutStats)

	if afterPutStats.TotalRecords > afterBeginStats.TotalRecords {
		t.Logf("✓ Put operation was recorded in WAL")
	} else {
		t.Logf("✗ Put operation was NOT recorded in WAL")
	}
	...
	// 检查提交后的统计
	afterCommitStats := txnMgr.txnWAL.GetStatistics()
...
func TestDebugStorageAdapter(t *testing.T) {
	...
	// 检查适配器的WAL
	t.Logf("Storage adapter created successfully")
