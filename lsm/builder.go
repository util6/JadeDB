...
/*
LSM 树表构建器（Table Builder）模块

表构建器是 LSM 树中负责创建 SSTable 文件的核心组件。
它将内存中的有序数据转换为磁盘上的不可变 SSTable 文件。

核心功能：
1. 数据组织：将键值对组织成有序的数据块
2. 索引构建：创建块索引，支持快速的键查找
3. 压缩优化：可选的数据压缩，减少存储空间
4. 校验保护：生成校验和，确保数据完整性

设计原理：
- 分块存储：将数据分割成固定大小的块，支持随机访问
- 有序组织：确保键在文件内部和块内部都是有序的
- 索引分离：将索引信息集中存储，加速查找过程
- 元数据管理：记录文件的统计信息和配置参数

文件结构：
[数据块1][数据块2]...[数据块N][块索引][布隆过滤器][元数据][校验和]

构建流程：
1. 初始化：创建构建器并设置配置参数
2. 添加数据：按顺序添加键值对到当前块
3. 块管理：当块达到大小限制时，完成当前块并创建新块
4. 索引构建：为每个块创建索引条目
5. 文件生成：将所有数据写入磁盘文件
6. 元数据写入：添加索引、布隆过滤器和元数据
...
type tableBuilder struct {
	...
	// sstSize 是正在构建的 SSTable 的预估总大小（字节）。
	// 用于控制文件大小，防止单个文件过大。
	// 在构建过程中动态更新。
	sstSize int64
	...
	// opt 包含构建 SSTable 的配置参数。
	// 包括块大小、压缩选项、布隆过滤器配置等。
	opt *Options

	// 数据存储

	// blockList 存储所有已完成的数据块。
	// 这些块将按顺序写入到 SSTable 文件中。
	// 每个块都包含有序的键值对数据。
	blockList []*block

	// 统计信息

	// keyCount 记录已添加到表中的键的总数量。
	// 用于统计和验证目的。
	keyCount uint32
